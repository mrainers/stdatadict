...
#'
#' The preparation procedure includes the following steps:
#' - A 'hidden' variable is added if not already present
#' - Entries from older study versions are removed.
...
#' - formtablename entries are cleared from the '(e)mnp<project_id>' string.
#'
#' @param fs table from secutrial metadata
#' @param vpfs table from secutrial metadata
#' @param project_id character string extracted from 'formtablename' variable that
...
#' @noRd
refine_fs <- function(fs, vpfs, project_id) {
  if ("hidden" %notin% names(fs)) {
...
#' - (sub)formtablename entries are cleared from the '(e)mnp<project_id>' string.
#'
...
#' @noRd
refine_qs <- function(qs, project_id, invert_hidden = NULL) {
    if ("hidden" %notin% names(qs)) {
      qs <- qs |> mutate(hidden = as.numeric(NA), .after = "formtablename")
    }
  ...
  qs |>
    ...
    mutate(
      ...
      formtablename = str_remove(.data$formtablename, str_c("mnp", project_id)),
      subformtablename = str_remove(.data$subformtablename, str_c("mnp", project_id))
    )
}
...
refine_is <- function(is, questions) {
  if ("hidden" %notin% names(is)) {
    is <- is |> mutate(hidden = as.numeric(NA), .after = "formtablename")
  }
...
#' - formtable names are cleaned from "mnp<project_id>" strings.
...
#' @noRd
refine_cl <- function(cl, project_id) {
  cl %>%
    filter(str_detect(.data$column, str_c("mnp", project_id))) |>
    mutate(column = str_remove(.data$column, str_c("mnp", project_id))) |>
    mutate(
      tablename = str_extract(.data$column, "^[^\\.]*") |>
        str_remove(str_c("mnp", project_id)),
...
create_datadict_tables <- function(st_metadata, ...) {
  ...
  project_id <- get_project_id(st_metadata$fs)
  intermediates$project_id <- project_id
  ...
  forms <- refine_fs(st_metadata$fs, st_metadata$vpfs, project_id)
  intermediates$forms <- forms

  questions <- refine_qs(st_metadata$qs, project_id, invert_hidden)
  intermediates$questions <- questions

  items <- refine_is(st_metadata$is, questions)
  intermediates$items <- items

  vallabs <- refine_cl(st_metadata$cl, project_id)
...
join_with_form_items <- function(datadict_tables, data, ...) {
  ...
  form_items_joined <- purrr::reduce(
    .x = data_transl,
    .f = ~ .x |> rlang::exec("left_join", x = _, y = .y, !!!dots),
  ...
  unite_vars <- data_transl |>
    map(\(df) names(df) |> setdiff(form_item_names)) |>
    unlist() |>
    tibble::tibble(var = _) |>
